...
export default async function decorate(block) {
  ...
  const SectorCartBanner = ({ blockColor, blockTitle, blockDescription, blockCTA, blockImage }) => html`
    <div class="cart-banner-wrapper" style="background-color: ${blockColor}">
      ...
      <div class="cart-banner-content">
        ...
        <a href="${blockCTA.href}" class="cart-banner-cta">${blockCTA.label}</a>
      </div>
    </div>
  `;
  ...
  let blockColor = '', blockTitle = '', blockDescription = '', blockCTA = null, blockImage = null;
  ...
  while (items.length) {
    ...
    if (isAHref(nextItem)) {
      ...
      if (label?.startsWith('#')) blockColor = label;
      else blockCTA = { href, label };
      items.shift();
      ...
      if (text === 'commerce-cart-banner-end') continue;
      if (!blockTitle) blockTitle = text;
      else blockDescription = text;
    }
  }

  const SectorCartBannerApp = html`<${SectorCartBanner} blockColor=${blockColor} blockTitle=${blockTitle} blockDescription=${blockDescription} blockCTA=${blockCTA} blockImage=${blockImage} />`;
  ...
  Prender(SectorCartBannerApp, block);
